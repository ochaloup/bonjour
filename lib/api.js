...
router.routes = (store) => {
  ...
  router.get('/bonjour', (req, resp) => {
    console.log(req.query.tmEnlistUri)
    min = Math.ceil(1);
    max = Math.floor(Number.MAX_SAFE_INTEGER);
    var pId = Math.floor(Math.random() * (max - min)) + min;
    console.log("pid: " + pId)
    var headerString = `<http://bonjour:8080/api/${pId}/participant>; rel="participant",<http://bonjour:8080/api/${pId}/terminator>; rel="terminator"`;
    console.log("header: " + headerString);

    request({
          url: req.query.tmEnlistUri,
          method: "POST",
          headers: {
          ...
          console.log("recovery 'location': " + response.headers.location);
          console.log("error: " + error)
      }
    );

    resp.type('text/plain').send(sayBonjour())
  });

  router.put('/:pid/terminator', (req, resp) => {
